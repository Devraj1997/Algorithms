...
public class FarthestInFuture {
	...
	public static void initialization() {
		// if the request array is less than the cache size, it adds all the elements it
		// needs to into the cache
		if (MemArr.length < 3) {
			for (int i = 0; i < MemArr.length; i++) {
				cache[i] = MemArr[i];
			}
		}
		// otherwise, the first three elements of the request array are added into the
		// cache
		else {
	...
	// checks if the request is in the cache, if it is it returns true, else returns
	// false
	...
	public static void updateCache(int x, int index) {
		...
		// if any of the indexes are still 0, that cache place is the one to get
		// replaced
		...
		// otherwise, it checks whichever index is the largest and sets the respective
		// cache
	...
	public static void serviceRequest() {
		...
		for (int i = 3; i < MemArr.length; i++) {
			// for every element in the request array, it prints the cache and checks the
			// hits and
			...
			if (isHit(MemArr[i])) {
				...
				// if isHit returns false, it increments the miss counter, and calls to update
				// the
				// cache array
				missCount++;
	...
	public static void main(String[] args) {
		...
		// service request is taken as a command line argument, if there aren't any, it
